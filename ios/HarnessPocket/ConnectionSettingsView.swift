...
struct ConnectionSettingsView: View {
    ...
    var body: some View {
        Form {
            Section("接続状態") {
                Label(model.connected ? "DSHに接続しています" : "DSHへの接続を確認してください", systemImage: model.connected ? "checkmark.circle" : "wifi.exclamationmark")
                ...
                LabeledContent("再起動後の接続更新", value: automatic ? "自動" : "手動")
            ...
            Section {
                Text(automatic ? "DSHを再起動すると、最新の接続情報を自動で読み直します。少し待ってもつながらない時は「再接続する」を試してください。" : "DSHを起動してから「再接続する」を試してください。起動時URLが変わった時は、下の入力欄から更新できます。")
    ...
    private func refresh() async {
        ...
        catch { message = "DGXの中継に届いていません。TailscaleとDGXの起動状態を確認してください。" }
    }
    private func reconnect(useURL: Bool = false) async {
        ...
        do {
            ...
            for _ in 0..<8 {
                ...
                if model.connected { message = "接続できました。会話を続けられます。"; return }
            }
            message = "再接続を続けています。DSHが起動しているか確認してください。"
